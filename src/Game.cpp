...
std::array<Line, kLineCount> build_lines() {
  ...
  auto add_line = [&](int x0, int y0, int z0, int dx, int dy, int dz) {
    Line line{};
    for (int step = 0; step < N; ++step) { line.cells[step] = {x0 + step * dx, y0 + step * dy, z0 + step * dz}; }
    lines[index++] = line;
  };
...
bool board_full(const Game &game) {
  for (int x = 0; x < N; ++x) {
    for (int y = 0; y < N; ++y) {
      for (int z = 0; z < N; ++z) {
        if (game.board[x][y][z] == Cell::Empty) { return false; }
      }
    }
  }
  return true;
...
bool Game::make_move(int x, int y, int z) {
  ...
  if (history_cursor < history_size) { history_size = history_cursor; }
...
void Game::rebuild_board_from_history() {
  for (int x = 0; x < N; ++x) {
    for (int y = 0; y < N; ++y) {
      for (int z = 0; z < N; ++z) { board[x][y][z] = Cell::Empty; }
    }
  }
...
void Game::recompute_state() {
  ...
  for (int x = 0; x < N; ++x) {
    for (int y = 0; y < N; ++y) {
      for (int z = 0; z < N; ++z) { highlighted[x][y][z] = false; }
    }
  }
  ...
  for (const Line &line : kLines) {
    ...
    if (first_line_owner == Cell::Empty) { first_line_owner = owner; }
    if (owner == Cell::X) {
      ++x_line_count;
    } else {
      ++o_line_count;
  ...
  if (count_all_lines) {
    if (full) {
      ...
      if (x_line_count != o_line_count) {
        const bool x_wins = misere_mode ? (x_line_count < o_line_count) : (x_line_count > o_line_count);
      ...
      for (const Line &line : kLines) {
        ...
        for (const auto &cell : line.cells) { highlighted[cell[0]][cell[1]][cell[2]] = true; }
      }
    }
    return;
  }
